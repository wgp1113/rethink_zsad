...
import cv2
from config import DATA_ROOT3


class DataSolver:
    def __init__(self, root, clsnames, dataset_name='', exclude_dataset_name='', training=True):
        self.root = root
        self.clsnames = clsnames

        self.path = os.path.join(self.root, 'meta.json')

        if 'atc' in dataset_name:
            self.path = os.path.join(root, f'{dataset_name}_{exclude_dataset_name}_meta.json')
        print(self.path)
        # if 'atc' in dataset_name:
        #     self.path = os.path.join(root, f'{dataset_name}_{exclude_dataset_name}_meta.json')
...
class BaseDataset(data.Dataset):
    def __init__(self, clsnames, transform, target_transform, root, aug_rate=0., training=True, dataset_name='', exclude_dataset_name=''):
        self.root = root
        ...
        solver = DataSolver(root, clsnames, dataset_name, exclude_dataset_name, self.training)
