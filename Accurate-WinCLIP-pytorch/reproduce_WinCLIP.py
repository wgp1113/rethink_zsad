...
def test(args,):
    ...
    pretrained_path = '/home/dell/wgp/pretrained_model/clip-vit-large-patch14-336'

    model = CLIPModel.from_pretrained(pretrained_path)
    lora_config = LoraConfig(
        r=8,  # LoRA 的秩
        lora_alpha=16,  # LoRA 的缩放因子
        target_modules=['v_proj','k_proj'],  # 需要添加 LoRA 的模块
    ...
    if dataset_name == 'mvtec':
        ...
        test_data = RealIADDataset(root=dataset_dir, transform=preprocess, target_transform=transform, mode='test')
